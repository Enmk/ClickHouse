...
/** TimeZoneImpl allows for various date/time related operations in a given timezone.
 *
 ...
 *
 * Exact range depends on TimeZoneImpl::luts_size, for value of 5 that is roughly from 1698 to 2378.
 *
 ...
 *    method is worse than corresponding method of DateLUTImpl.
 */
class TimeZoneImpl
{
public:
    explicit TimeZoneImpl(const std::string & timezone_name);
    ~TimeZoneImpl();
...
private:
    ...
    LutEntry luts[luts_size + 1]; // +1 since last is a dummy value used for determinig TimeZoneImpl range.
};
