...
TimeZoneImpl::TimeZoneImpl(const std::string & timezone_name)
      : default_lut(timezone_name)
{
...
TimeZoneImpl::~TimeZoneImpl()
{
    for (size_t i = 0; i < sizeof(luts)/sizeof(luts[0]); ++i)
    {
        // LUT-0, not owned by TimeZoneImpl via luts
        if (i - luts_size / 2 == 0)
            continue;
...
const DateLUTImpl & TimeZoneImpl::getLUTByIndexMaybeWithLock(Int32 lut_index) const
{
