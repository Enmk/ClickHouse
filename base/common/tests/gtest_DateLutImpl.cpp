...
TEST_P(TimeZone_VS_DateLUT_Test, SameAsDateLUTImpl)
{
    ...
    const auto lut = std::make_unique<DateLUTImpl>(timezone_name);
    const auto tz = std::make_unique<TimeZone>(timezone_name);
...
TEST(DateLUTTest, NonZeroOffset)
{
    // time_t pointing to 2169-11-13 08:00 UTC
    // NOTE: even though offest points to a certain time, LUT will have the first entry at 2169-01-01 00:00
    const Int64 LUT_OFFSET = 6307'228'800ll;
    const Int64 FIRST_DAY_TIME_T = 6279'897'600ll; // 2169-01-01 00:00
    const auto lut = std::make_unique<DateLUTImpl>("UTC", LUT_OFFSET);

//    SCOPED_TRACE(LUT_OFFSET);

    EXPECT_EQ(lut->getDateLutMin(), FIRST_DAY_TIME_T);
    EXPECT_EQ(lut->toGlobalDayNum(DayNum(0)), 72684);

    //2169-01-01
    const auto & v = lut->getValues(DayNum(0));
    EXPECT_EQ(v.date, FIRST_DAY_TIME_T);
    EXPECT_EQ(v.year, 2169);
    EXPECT_EQ(static_cast<UInt32>(v.month), 1);
    EXPECT_EQ(static_cast<UInt32>(v.day_of_month), 1);

    // 2169-01-02
    const auto & v1 = lut->getValues(DayNum(1));
    EXPECT_EQ(v1.year, 2169);
    EXPECT_EQ(static_cast<UInt32>(v1.month), 1);
    EXPECT_EQ(static_cast<UInt32>(v1.day_of_month), 2);

    // 2169-01-31
    const auto & v3 = lut->getValues(DayNum(v.days_in_month - 1));
    EXPECT_EQ(v3.year, 2169);
    EXPECT_EQ(static_cast<UInt32>(v3.month), 1);
    EXPECT_EQ(static_cast<UInt32>(v3.day_of_month), 31);

    EXPECT_EQ(lut->toDate(DayNum(0)), FIRST_DAY_TIME_T);
    EXPECT_EQ(lut->toDayNum(FIRST_DAY_TIME_T), 0);

    EXPECT_EQ(lut->toNumYYYYMMDD(DayNum(0)), 21690101);
    EXPECT_EQ(lut->toNumYYYYMMDDhhmmss(FIRST_DAY_TIME_T), 21690101000000);

    EXPECT_EQ(lut->timeToString(FIRST_DAY_TIME_T), "2169-01-01 00:00:00");
    EXPECT_EQ(lut->dateToString(DayNum(0)), "2169-01-01");

    EXPECT_EQ(lut->toYear(DayNum(0)), 2169);
    EXPECT_EQ(lut->toMonth(DayNum(0)), 01);
    EXPECT_EQ(lut->toDayOfMonth(DayNum(0)), 01);
}

TEST(DateLUTTest, TimeValuesInMiddleOfRange)
{
    const auto lut = std::make_unique<DateLUTImpl>("Europe/Minsk");
...
{
    const auto lut = std::make_unique<DateLUTImpl>("UTC");
    const time_t time = 0;
    ...
    // Some of those return values outside of LUT, producing bogus results
    // but we document those results nevertheless to retain/check compatibility.
    EXPECT_EQ(lut->toFirstDayOfWeek(time), 4323283200 /*time_t*/);
    ...
    EXPECT_EQ(lut->toFirstDayOfPrevMonth(time), 4323283200 /*time_t*/);
    ...
    EXPECT_EQ(lut->toFirstDayOfISOYear(time), 4323283200 /*time_t*/);
...
{
    // Most of the output values validated with python's datetime and pytz, like this:
    // datetime.fromtimestamp(4294349245, pytz.utc)

    const auto lut = std::make_unique<DateLUTImpl>("UTC");
    const time_t time = 4294349245; // 2106-01-31 02:47:25
    SCOPED_TRACE(time);
    ...
    EXPECT_EQ(lut->toDayOfWeek(time), 7);
...
//    EXPECT_EQ(lut->toFirstDayOfPrevMonth(time), 4291747200 /*time_t*/);
    ...
    EXPECT_EQ(lut->daysInMonth(time), 31 /*UInt8*/);
    EXPECT_EQ(lut->toDateAndShift(time, 10), 4295203200 /*time_t*/);

    EXPECT_EQ(lut->toTime(time), 10045 /*time_t*/); // 1970-01-01 02:47:25
    EXPECT_EQ(lut->toHour(time), 2);
    EXPECT_EQ(lut->toMinute(time), 47);
    EXPECT_EQ(lut->toSecond(time), 25);

    EXPECT_EQ(lut->toStartOfMinute(time), 4294349220 /*time_t*/);
    EXPECT_EQ(lut->toStartOfFiveMinute(time), 4294349100 /*time_t*/);
    EXPECT_EQ(lut->toStartOfFifteenMinutes(time), 4294349100 /*time_t*/);
    EXPECT_EQ(lut->toStartOfTenMinutes(time), 4294348800 /*time_t*/);
    EXPECT_EQ(lut->toStartOfHour(time), 4294346400 /*time_t*/);
    ...
    EXPECT_EQ(lut->toFirstDayOfISOYear(time), 4292006400 /*time_t*/); // 2106-01-04
//    EXPECT_EQ(lut->toISOWeek(time), 7102 /*unsigned*/); // ???
    ...
    EXPECT_EQ(lut->toRelativeHourNum(time), 1192874 /*time_t*/);
    EXPECT_EQ(lut->toRelativeMinuteNum(time), 71572487 /*time_t*/);
    EXPECT_EQ(lut->toStartOfHourInterval(time, 5), 4294332000 /*time_t*/); // 2106-01-30 22:00
    EXPECT_EQ(lut->toStartOfMinuteInterval(time, 6), 4294348920 /*time_t*/); // 2106-01-31 02:42
    EXPECT_EQ(lut->toStartOfSecondInterval(time, 7), 4294349241 /*time_t*/); // 2106-01-31 02:47:21

    EXPECT_EQ(lut->toNumYYYYMM(time), 210601 /*UInt32*/);
    EXPECT_EQ(lut->toNumYYYYMMDD(time), 21060131 /*UInt32*/);
    EXPECT_EQ(lut->toNumYYYYMMDDhhmmss(time), 21060131024725 /*UInt64*/);

    EXPECT_EQ(lut->addDays(time, 10), 4295213245 /*time_t*/); // 2106-02-10 02:47:25
    EXPECT_EQ(lut->addWeeks(time, 1), 4294954045 /*time_t*/); // 2106-02-07 02:47:25
    EXPECT_EQ(lut->addMonths(time, 1), 4296768445 /*time_t*/); // 2106-02-28 02:47:25
    EXPECT_EQ(lut->addQuarters(time, 1), 4302038845 /*time_t*/);   // 2106-04-30 02:47:25
    //EXPECT_EQ(lut->addYears(time, 1), 3666394045 /*time_t*/); // BOGUS VALUE (out of LUT range)

    EXPECT_EQ(lut->timeToString(time), "2106-01-31 02:47:25" /*std::string*/);
...
TEST_P(DateLUTRangeTest, UTC)
{
    ...
    const auto & lut = DateLUT::getTimeZone("UTC");
...
TEST(TimeZoneTest, getLUTIndex)
{
    ...
    const auto lut = std::make_unique<DateLUTImpl>("Europe/Minsk");
    const ExtendedDateLUTImpl tz(*lut);
...
TEST(TimeZoneTest, TimeValuesInMiddleOfRange)
{
    const auto default_lut = std::make_unique<DateLUTImpl>("Europe/Minsk");
    const ExtendedDateLUTImpl lut(*default_lut);
    const time_t time = 1568650811; // 2019-09-16 19:20:11 (Monday)
...
TEST_P(TimeZoneRangeTest, UTC)
{
    ...
    const auto lut = std::make_unique<DateLUTImpl>("UTC");
    const auto & tz = ExtendedDateLUTImpl(*lut);
    const auto & [begin, end, step] = GetParam();
    ...
    for (time_t expected_time_t = begin; begin < end ? expected_time_t < end : expected_time_t > end; expected_time_t += step, ++i)
    {
        ...
        const auto & lut_by_index = tz.getLUTByIndex(lut_index);
        SCOPED_TRACE(::testing::Message{} << "Lut index: " << lut_index << "\tmin: " << lut_by_index.getDateLutMin());
