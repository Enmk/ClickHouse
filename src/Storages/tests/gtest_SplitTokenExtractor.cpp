...
using TokenExtractorCreator = std::function<std::unique_ptr<ITokenExtractor>(void)>;

struct TokenExtractorTestCase
{
...
std::ostream & operator<<(std::ostream & ostr, const TokenExtractorCreator & creator)
{
    const auto c = creator();
    const auto & cr = *c;
    return ostr << typeid(cr).name();
}

std::ostream & operator<<(std::ostream & ostr, const TokenExtractorTestCase & test_case)
{
    return ostr << test_case.description;
}

template <typename Extractor>
std::unique_ptr<ITokenExtractor> make()
{
    return std::make_unique<Extractor>();
}

class TokenExtractorTest : public ::testing::TestWithParam<
        std::tuple<TokenExtractorCreator, TokenExtractorTestCase>>
{
public:
    void SetUp() override
    {
        const auto & [creator, param] = GetParam();
        const auto & source = param.source;
        data = std::make_unique<PaddedPODArray<char>>(source.data(), source.data() + source.size());
        token_extractor = creator();
...
TEST_P(TokenExtractorTest, nextInColumn)
{
    const auto & [_, param] = GetParam();
...
INSTANTIATE_TEST_SUITE_P(NoTokens,
    TokenExtractorTest,
    ::testing::Combine(
        ::testing::Values(&make<SplitTokenExtractor>, &make<SplitTokenExtractor2>),
        ::testing::ValuesIn(std::initializer_list<TokenExtractorTestCase>{
            {
...
INSTANTIATE_TEST_SUITE_P(SingleToken,
    TokenExtractorTest,
    ::testing::Combine(
    ::testing::Values(make<SplitTokenExtractor>),
    ::testing::ValuesIn(std::initializer_list<TokenExtractorTestCase>{
        {
            "Single-char token",
            "f",
        ...
        {
            "Single-char surrounded by whitespace",
            "f",
            {"f"}
        },
        {
        ...
        {
            "Medium single token surruonded by whitespace",
            "\t\vfoobar\n\r",
            {"foobar"}
        },
        {
        ...
        {
            "Long single token surruonded by whitespace",
        ...
        {
            "Longer single token surruonded by whitespace",
        ...
        {
            "Enormous single token surruonded by whitespace",
            "\t\v"
            "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
            "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
            "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
...
INSTANTIATE_TEST_SUITE_P(SingleTokens_UnderscoresShort,
    TokenExtractorTest,
    ::testing::Combine(
    ::testing::Values(make<SplitTokenExtractor2>),
    ::testing::ValuesIn(std::initializer_list<TokenExtractorTestCase>{
        {
            "Single token",
            "f_o",
        ...
        {
            "Single token that begins with underscore",
            "_fo",
            {"_fo"}
        },
        {
            "Single token that ends with underscore",
            "fo_",
            {"fo_"}
        },
        {
            "Single token with underscore",
            "_fo_",
            {"_fo_"}
        },
        {
        ...
        {
            "Single token with whitespace",
            " f_o ",
            {"f_o"}
        },
        {
            "Single token that begins with underscore with whitespace",
            " _fo ",
            {"_fo"}
        },
        {
            "Single token that ends with underscore with whitespace",
            " fo_ ",
            {"fo_"}
        },
        {
            "Single token with underscore with whitespace",
            " _fo_ ",
            {"_fo_"}
        },
        {
...
INSTANTIATE_TEST_SUITE_P(SingleTokens_UnderscoresLong,
    TokenExtractorTest,
    ::testing::Combine(
    ::testing::Values(make<SplitTokenExtractor2>),
    ::testing::ValuesIn(std::initializer_list<TokenExtractorTestCase>{
        ...
        {
            "Single token begins with underscore",
            "_foobarfoo_foobarfoo",
            {"_foobarfoo_foobarfoo"}
        },
        {
            "Single token ends with underscore",
            "foobarfoo_foobarfoo_",
            {"foobarfoo_foobarfoo_"}
        },
        {
            "Single token surrounded by underscores",
            "_foobarfoo_foobarfoo_",
            {"_foobarfoo_foobarfoo_"}
        },
        {
            "Single token with whitespaces",
            " foobarfoofoobarfoo ",
            {"foobarfoofoobarfoo"}
        },
        {
            "Single token begins with underscore with whitespaces",
            " _foobarfoo_foobarfoo ",
            {"_foobarfoo_foobarfoo"}
        },
        {
            "Single token ends with underscore with whitespaces",
            " foobarfoo_foobarfoo_ ",
            {"foobarfoo_foobarfoo_"}
        },
        {
            "Single token surrounded by underscores with whitespaces",
            " _foobarfoo_foobarfoo_ ",
            {"_foobarfoo_foobarfoo_"}
        }
    }))
