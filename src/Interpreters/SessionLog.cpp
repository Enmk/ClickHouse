...
DB::SessionLogElement makeSessionLogElement(DB::SessionLogElementType type, DB::UserPtr user, const DB::ClientInfo & client_info)
{
    DB::SessionLogElement result(type, user->getName());
    result.client_info = client_info;
    result.user = user->getName();
    result.user_identified_with = user->authentication.getType();
    result.external_auth_server = user->authentication.getLDAPServerName();
...
SessionLogElement::SessionLogElement(Type type_, String user_)
    : type(type_),
      user(std::move(user_))
{
...
NamesAndTypesList SessionLogElement::getNamesAndTypes()
{
    ...
    auto identified_with_column = std::make_shared<DataTypeEnum8>(
        DataTypeEnum8::Values
        {
            {"NO_PASSWORD",            static_cast<Int8>(AuthType::NO_PASSWORD)},
            {"PLAINTEXT_PASSWORD",     static_cast<Int8>(AuthType::PLAINTEXT_PASSWORD)},
            {"SHA256_PASSWORD",        static_cast<Int8>(AuthType::SHA256_PASSWORD)},
            {"DOUBLE_SHA1_PASSWORD",   static_cast<Int8>(AuthType::DOUBLE_SHA1_PASSWORD)},
            {"LDAP",                   static_cast<Int8>(AuthType::LDAP)},
            {"KERBEROS",               static_cast<Int8>(AuthType::KERBEROS)}
        });
    ...
    auto changed_settings_type_column = std::make_shared<DataTypeArray>(
    ...
    return
    {
        ...
        {"profiles", std::make_shared<DataTypeArray>(
            std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()))},
        {"roles", std::make_shared<DataTypeArray>(
            std::make_shared<DataTypeLowCardinality>(std::make_shared<DataTypeString>()))},
...
void SessionLogElement::appendToBlock(MutableColumns & columns) const
{
    ...
    columns[i++]->insertData(user.data(), user.length());
...

    {
        ...
        auto & names_col = assert_cast<ColumnString &>(*changed_settings_tuple_col.getColumnPtr(0)->assumeMutable());
...
void SessionLog::addLoginSuccess(const ContextAccess & access, const ClientInfo & client_info, const Settings & settings)
{
    const auto user = access.getUser();
    auto log_entry = makeSessionLogElement(SESSION_LOGIN_SUCCESS, user, client_info);
    if (const auto roles_info = access.getRolesInfo())
        log_entry.roles = roles_info->getCurrentRolesNames();

    log_entry.profiles = access.getCurrentProfileNames();
...
void SessionLog::addLoginFailure(const ClientInfo & info, const String & user, const String & reason)
{
    SessionLogElement result(SESSION_LOGIN_FAILURE, user);

    result.auth_failure_reason = reason;
    result.client_info = info;
    result.user_identified_with = Authentication::Type::NO_PASSWORD;

    add(result);
}

void SessionLog::addLogOut(const ContextAccess & access, const ClientInfo & client_info)
{
    add(makeSessionLogElement(SESSION_LOGOUT, access.getUser(), client_info));
}
