...
struct SessionLogElement
{
    ...
    SessionLogElement(Type type, String user);
...
public:
    void addLoginSuccess(const ContextAccess & access, const ClientInfo & client_info, const Settings & settings);
    void addLoginFailure(const ClientInfo & info, const String & user, const String & reason);
    void addLogOut(const ContextAccess & access, const ClientInfo & client_info);
};
