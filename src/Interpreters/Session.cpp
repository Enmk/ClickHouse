      ...
      initial_session_context(session_context)
{
    ...
    session_context->setSession(this);
...
Session::~Session()
{
    ...
    if (access)
    {
        auto session_log = session_context->getSessionLog();
        auto user = access->getUser();
        if (session_log && user)
            session_log->addLogOut(*access, getClientInfo());
    }
}
...
void Session::setUser(const Credentials & credentials, const Poco::Net::SocketAddress & address)
{
    try
    {
        ...
        if (session_log && access && access->getUser() && dynamic_cast<const BasicCredentials *>(&credentials))
        {
            session_log->addLoginSuccess(*access, getClientInfo(), session_context->getSettingsRef());
        }
    }
    catch (const std::exception & e)
    {
        onLogInFailure(credentials.getUserName(), e.what());
        throw;
    }
}
...
void Session::onLogInFailure(const String & user_name, const String & failure_reason)
{
    if (auto session_log = getSessionLog())
        session_log->addLoginFailure(getClientInfo(), user_name, failure_reason);
}
...
void Session::promoteToNamedSession(const String & session_id, std::chrono::steady_clock::duration timeout, bool session_check)
{
    ...
    session_context->makeSessionContext();
    new_named_session->context->setSession(this);
...
void Session::releaseNamedSession()
{
    if (named_session)
    {
        named_session->context->setSession(nullptr);
...
std::shared_ptr<SessionLog> Session::getSessionLog()
{
    return session_context->getSessionLog();
}
