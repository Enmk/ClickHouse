...
(
    id UInt64,
    s String,
    INDEX tok_bf (s, lower(s)) TYPE tokenbf_v2(512, 3, 0) GRANULARITY 1
...
SELECT max(id) FROM bloom_filter WHERE hasToken_v2(s, 'abc');
SELECT max(id) FROM bloom_filter WHERE hasToken_v2(s, 'def');
SELECT max(id) FROM bloom_filter WHERE hasToken_v2(s, 'zzz');
...
SELECT max(id) FROM bloom_filter WHERE hasToken_v2(s, 'yyy'); -- { serverError 158 }

-- this syntax is not supported by tokenbf
SELECT max(id) FROM bloom_filter WHERE hasToken_v2(s, 'zzz') == 1; -- { serverError 158 }
